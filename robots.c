...
int	main_game() {
	...
	switch(c) {
		...
		case	'i': dx= 0,dy=0; break;
		case	'o': dx= 1,dy=0; break;
		case	'j': dx=-1,dy=1; break;
...
void	instruction() {
		...
		printw("## --- Rock , which robot dies if robots touch.\n");
		printw("<> --- Raging Robot , which chases you step by step\n");
		printw("() --- You , control and run away from the robots.\n");
		printw("             if you touch robots and rocks then you die.\n");
		printw("Key Control: 7   8   9\n\n");
		printw("             u   i   o\n\n");
		printw("             j   k   l\n\n");
		printw("             these keys move you to each direction.\n");
		printw("             and 'q' key to quit.\n");
		printw("             Good Luck\n");
		printw("Hit a key to start gamne\n");
		getch();
	}
