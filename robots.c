...
int enemyl[40];   //  ロボットが生きているかのフラグ
int enemyx[40];   //  ロボットｘ座標
int enemyy[40];   //  ロボットｙ座標

int rockl[320];   //  岩があるかどうかのフラグ
int rockx[320];   //  岩ｘ座標
int rocky[320];   //  岩ｙ座標

int enemymax=12;  // 最初のロボットの数
...
void  move_robots() {
  ...
  for(int i=0;i<enemymax;i++) {
    if (enemyl[i]==0)   // ロボットが死んでいたら、スキップ
          continue;
    //
    ...
    enemyx[i]=x,enemyy[i]=y;  // ロボットを一歩進める
    ...
    if (enemyl[i])    // ロボットが生きているか？
            putchara(enemyx[i],enemyy[i],ENEMY);  // ロボットを描画
    else
...
void  instruction() {
    ...
    printw("%s -- プレイヤー  , キーで操作し、岩にロボットを誘って消滅させよう。\n",PLAYER);
    printw("\n");
