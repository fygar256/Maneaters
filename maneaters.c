...
void  move_maneaters() {
  ...
  for(int i=0;i<enemymax;i++) {
    ...
    if (enemyl[i])    // 敵が生きているか？
            ...
            putchara(enemyx[i],enemyy[i],SPACE);  // 空白を描画
...
void  instruction() {
    ...
    printw("Maneaters Ver 0.9\n"); // printwで、stdscrに出力します。

    printw("Mission : survive from Maneaters.\n");
    printw("%s -- Maneater, which chases player step by steop\n",ENEMY);
    printw("%s -- Rock, Player and Maneater die when touch. \n",ROCK);
    printw("%s -- Player, Control with the keys and lead maneaters to\n",PLAYER);
    printw("              touch the rocks to die.\n",PLAYER);
    printw("\n");
    ...
    printw(" ｊ  ｋ   ｌ           １  ２  ３   \n");
    printw("\n");
    printw(" 'i' and '5' are for player not to move and only for maneaters to move\n");
    printw(" This program stops by Ctrl-C.\n");
    printw("             Good Luck\n");
